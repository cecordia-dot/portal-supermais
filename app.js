...
async function deleteProduct(id){
  ...
  if(!confirm(`Desativar "${p.name}" do catálogo?`))return;
  try{await api(`/api/products/${p.id}`,{method:'DELETE'});await refreshData();toast('Produto desativado.');render()}catch(e){toast(e.message,true)}
}
